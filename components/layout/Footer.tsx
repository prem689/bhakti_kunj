...
import { Facebook, Instagram, Youtube, Linkedin, Phone, Mail, Clock } from 'lucide-react';
  ...
  return (
    <footer style={{ background: 'var(--bg-primary)', borderTop: '1px solid rgba(200,169,107,0.12)' }}>
      <div className="max-w-360 mx-auto px-6 lg:px-16 py-16 lg:py-20">
        ...
        {/* Bottom Info */}
        <div className="pt-8 grid grid-cols-1 md:grid-cols-3 gap-8 mb-8" style={{ borderTop: '1px solid rgba(200,169,107,0.1)' }}>
          {[
            { title: 'REGISTERED OFFICE', lines: ['Sector 123, New City', 'District, State – 12345'] },
            { title: 'SALES OFFICE', lines: ['Grand Central Mall', 'Sector 123, New City'] },
            { title: 'RERA DETAILS', lines: ['RERA Reg. No: [XXXX]', 'License Valid until: [DATE]'] },
          ].map(({ title, lines }) => (
            <div key={title}>
